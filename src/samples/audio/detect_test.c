 ...
 * Assumes S16_LE mono at 16 kHz.
 * Keyword triggers are driven exclusively via the wov_test_1NN kcontrol switch.
 */
				...
				uint32_t frames)
{
	...
	if (cd->config.load_mips) {
		uint32_t cycles_per_frame = (cd->config.load_mips * 1000000 * frames)
...
#define SOF_WOV_CTRL_INDEX_MUTE 0
#define SOF_WOV_CTRL_INDEX_TEST 1
	...
	case SOF_IPC4_SWITCH_CONTROL_PARAM_ID: {
		...
		if (cp->id != 0) {
			/* Test trigger switch (non-zero control index): value=1 immediately triggers keyword detection */
			if (cp->chanv[0].value != 0) {
				comp_warn(dev, "kd_test: TEST SWITCH TRIGGER for slot %u (ctl_id=%u)!",
					  (uint32_t)cd->wov_slot_id, cp->id);
				if (!cd->drain_req)
				...
				/* Reset the test switch kcontrol back to 0 (off) */
				notify_control_change(dev, cp->id, 0);
			}
			return 0;
		}

		/* Default / id == 0: Mute switch: value=0 disables detection, value=1 enables it. */
					 ...
					 char *data)
{
	...
	case SOF_IPC4_SWITCH_CONTROL_PARAM_ID: {
		...
		*data_offset = resp_size;
		...
		/* Test trigger switch always resets to 0; mute switch returns armed state */
		if (ctl_id != 0)
			cp->chanv[0].value = 0;
		else
			cp->chanv[0].value = cd->muted ? 0 : 1;
		return 0;
	}
	default:
		return -EINVAL;
...
#if CONFIG_IPC_MAJOR_4
	/* Assign slot_id based on instance ID (instance 2->slot 0, instance 1->slot 1, instance 0->slot 2) */
	cd->wov_slot_id = (uint8_t)((KD_MAX_SLOTS - 1) -
				    (IPC4_INST_ID(dev->ipc_config.id) % KD_MAX_SLOTS));

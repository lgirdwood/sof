		...
		if (ret == 0 && data_ptr) {
			...
			if (last_inference_timer > 0 && stride_delta >= (38400ULL * 500ULL)) {
				...
				if (ret >= 0) {
					...
					static uint32_t category_totals[TFLM_CATEGORY_COUNT] = {0, 0, 0, 0};
					static uint32_t total_inferences = 0;

					total_inferences++;
					if (max_idx >= 0 && max_idx < TFLM_CATEGORY_COUNT)
						category_totals[max_idx]++;
					...
					snprintk(result_buf, sizeof(result_buf), "TFLM top prediction: %s confidence=%d pct (inferences=%u): silence=%u, unknown=%u, yes=%u, no=%u",
						 prediction[max_idx], max_pct, total_inferences,
						 category_totals[0], category_totals[1], category_totals[2], category_totals[3]);

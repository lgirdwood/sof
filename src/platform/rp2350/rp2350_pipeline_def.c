...
static const uint32_t rp2350_default_iir_coef_2ch[51] = {
...
static const uint32_t rp2350_default_drc_coef[35] = {
 ...
 * ------------------------------------------------------------------------- */
static const struct sof_static_comp rp2350_comps[] = {
	...
	SOF_STATIC_COMP_MODULE(
		...
		.init_blob = rp2350_default_iir_coef_2ch,
		.init_blob_size = sizeof(rp2350_default_iir_coef_2ch)
	),
	SOF_STATIC_COMP_MODULE(
		.id = 4, .pipeline_id = 1, .name = "DRC_PB",
		.uuid = &drc_uuid, .direction = SOF_IPC_STREAM_PLAYBACK,
		.caps = SOF_STATIC_CAPS(SOF_IPC_FRAME_FLOAT, 48000, 2),
		.init_blob = rp2350_default_drc_coef,
		.init_blob_size = sizeof(rp2350_default_drc_coef)
	),
	...
	SOF_STATIC_COMP_MODULE(
		.id = 7, .pipeline_id = 2, .name = "EQ_CAP",
 ...
 * ------------------------------------------------------------------------- */
static const struct sof_static_route rp2350_routes[] = {
	/* Playback Chain: USB_PB (1) -> [1] -> VOL_PB (2) -> [2] -> EQ_PB (3) -> [3] -> DRC_PB (4) */
...
#if defined(CONFIG_RP2350_INTERFACE_PIO_I2S)
	/* DRC_PB (4) -> [4] -> DAI_I2S_PB (5) */
	SOF_STATIC_ROUTE(.src_comp_id = 4, .buffer_id = 4, .sink_comp_id = 5),

	/* DAI_I2S_CAP (6) -> [5] -> EQ_CAP (7) -> [6] -> VOL_CAP (8) -> [7] -> USB_CAP (9) */
...
#else
	/* Internal Loopback: DRC_PB (4) -> [4] -> EQ_CAP (7) -> [6] -> VOL_CAP (8) -> [7] -> USB_CAP (9) */
	SOF_STATIC_ROUTE(.src_comp_id = 4, .buffer_id = 4, .sink_comp_id = 7),
	SOF_STATIC_ROUTE(.src_comp_id = 7, .buffer_id = 6, .sink_comp_id = 8),
	SOF_STATIC_ROUTE(.src_comp_id = 8, .buffer_id = 7, .sink_comp_id = 9),
#endif
};
...
/* -------------------------------------------------------------------------
 * 4. PCM Stream Endpoints
 * ------------------------------------------------------------------------- */
static const struct sof_static_pcm rp2350_pcms[] = {
	SOF_STATIC_PCM(
		.pcm_id = 0,
...
/* -------------------------------------------------------------------------
 * 5. Kcontrols (Volume, Mute, EQ/DRC Bypass Switches)
 * ------------------------------------------------------------------------- */
static const struct sof_static_kcontrol rp2350_controls[] = {
	...
	SOF_STATIC_KCONTROL_SWITCH(
		.id = 4, .name = "Capture EQ Switch",
		.target_comp_id = 7,
		.def = 0,
		.uac2_entity_id = CAPTURE_EQ_FU_ID
	),
	SOF_STATIC_KCONTROL_VOLUME(
		.id = 5, .name = "Master Capture Volume",
		.target_comp_id = 8,
		.min = 0, .max = 65536, .def = 65536, .channels = 2,
 ...
 * ------------------------------------------------------------------------- */
static const struct sof_static_pipeline_desc rp2350_pipelines[] = {
	{
		.pipeline_id = 1,
 ...
 * ------------------------------------------------------------------------- */
const struct sof_static_topology g_rp2350_static_topology = {
	.name = "RP2350 Playback & Capture Pipeline",
	.num_pipelines = ARRAY_SIZE(rp2350_pipelines),
	.pipelines = rp2350_pipelines,
	.num_comps = ARRAY_SIZE(rp2350_comps),
	.comps = rp2350_comps,
	.num_buffers = ARRAY_SIZE(rp2350_buffers),
	.buffers = rp2350_buffers,
	.num_routes = ARRAY_SIZE(rp2350_routes),
	.routes = rp2350_routes,
	.num_pcms = ARRAY_SIZE(rp2350_pcms),
	.pcms = rp2350_pcms,
	.num_controls = ARRAY_SIZE(rp2350_controls),
	.controls = rp2350_controls,
};

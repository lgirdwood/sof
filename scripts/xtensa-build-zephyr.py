...
for arg in "$@"; do
  case "$arg" in
    ...
    *.s|*.S) IS_ASM=1; ARGS+=("$arg") ;;
    *) ARGS+=("$arg") ;;
  esac
done

# Use external assembler only for hand-written assembly files
if [ "$HAS_NO_INTEGRATED_AS" -eq 0 ] && [ "$IS_ASM" -eq 1 ]; then
...
def build_platforms():
	...
	for platform in args.platforms:
		...
		if args.llvm_clang:
			...
			# Use llvm toolchain variant for native Clang support
			...
			print("LLVM: Zephyr SDK support is disabled (native Clang only)")
			if "ZEPHYR_SDK_INSTALL_DIR" in platf_build_environ:
				del platf_build_environ["ZEPHYR_SDK_INSTALL_DIR"]
		elif args.zephyrsdk:
		...
		if args.llvm_no_sdk:
			build_cmd.append("-DLLVM_NO_ZEPHYR_SDK=ON")

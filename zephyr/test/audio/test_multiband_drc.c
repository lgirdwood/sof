...
static struct comp_dev *test_multiband_drc_create(void)
{
...
{
	struct comp_dev *comp = test_multiband_drc_create();
...
ZTEST(audio_multiband_drc, test_multiband_drc_config)
{
	struct comp_dev *comp = test_multiband_drc_create();
...
/* Use generated passthrough config blob for multiband DRC processing test */
#include "blobs/multiband_drc_passthrough.h"

ZTEST(audio_multiband_drc, test_multiband_drc_process)
{
	struct comp_dev *comp = test_multiband_drc_create();
	...
	if (comp->drv->ops.set_large_config) {
		int ret = comp->drv->ops.set_large_config(comp, 0, true, true,
			sizeof(multiband_drc_blob) - 32, (uint8_t *)multiband_drc_blob + 32);
